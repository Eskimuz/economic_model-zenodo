...
;  initial-labor-price
;  initial-capital-firms
;  initial-capital-farms
;  substitute-capital
...
to run-service-market
  ...
  ifelse (testing = true) [
    ...
    ifelse (any? demand) and (any? supply) [
      ...
      ask demand [
      ;; we add more distance for testing
        let close-supply supply in-radius (distance-setup * service-distance-multiplier)
        let black-demand demand with [color = black]
        if any? black-demand [ask black-demand [set close-supply supply]]
        set service-market-check (service-market-check + 1)
        ifelse any? close-supply [
          ...
          let lower-price [price] of low-price
          if (desires = 0) or ( wealth < lower-price) [
            set demand demand with [self != myself]
            stop
          ]
        ] [stop]
...
to  run-price-adjustment
  ...
  ask employers [
    ...
    if (labor-price / productivity) > price  [
      set price ((labor-price / productivity) + 0.5)
    ]
  ]
