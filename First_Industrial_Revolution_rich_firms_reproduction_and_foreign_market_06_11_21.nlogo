...
to setup-workers
  ...
  ask workers [
    ...
    set salary random-normal general-labor-price (general-labor-price / 5)
...
to setup-bourgeoisie
  ...
  ask bourgeoisie [
    set color orange
    set wealth random-normal (initial-capital / 3 * 2) (initial-capital / 8)
    set size 0.1
    set shape "person"
    setxy random-xcor random-ycor
...
to setup-farms
  ask farms [
    set capital random-normal initial-capital (initial-capital / 5)
    set shape "house"
    set color violet
    set size 0.2
    set productivity max (list 0.1 (random-normal farm-productivity (farm-productivity / 5))) ;historically accurate
    set labor-price random-normal general-labor-price (general-labor-price / 5)
    set price ((general-labor-price / productivity) + random-normal margin (margin / 5))
...
to setup-firms
  ...
  let mean-goods-price mean [price] of firms
    ask bourgeoisie [
    let pricing ( (mean-food-price + mean-goods-price) * random-float 5 )
    set price random-normal pricing (pricing / 5)
  ]
end
...
to run-dividends
  ...
  ask farms [
    if capital <= mean-salaries [
      ask out-own-neighbors [
        let transfer min ( list initial-capital ( random ( 0.5 * wealth)))
...
;    let actual-wealth (wealth)
    let random-number random-float 1
    if (wealth >= (2 * initial-capital / 3) and ( random-number < opening-chance))  [
      let random-capital (random-normal initial-capital 100)
      let initial-capital2 min (list random-capital wealth)
      set wealth (wealth - initial-capital2)
      set owner? true
      hatch-firms 1 [
        set agent-replacement-check (  agent-replacement-check + 1)
  ...
  let costs (trans-cost + initial-capital)
  let close in-radius distance-setup patches
  let rich-bougie bourgeoisie with [(wealth > (trans-cost + initial-capital)) and (any? close with [pcolor = black])]
    ask rich-bougie [
...
;      set deposited 0
  ...
  let richness (initial-capital * capital-rate-firm-reproduction * (average-salary-employed / general-labor-price ))
...
<experiments>
  <experiment name="experiment1" repetitions="10" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="general-labor-price">
      <value value="5"/>
      <value value="6"/>
      <value value="7"/>
      <value value="8"/>
    ...
    <steppedValueSet variable="trans-cost" first="1000" step="1000" last="5000"/>
